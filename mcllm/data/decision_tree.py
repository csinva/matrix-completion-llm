...
import torch
import random
from mcllm.data.util import get_masks, get_register_mask, get_att_mask, get_nan_mask
...
class RandomDecisionTreeNode:
    def __init__(self, depth, max_depth, n_features, feature_ranges, available_features):
        ...
        self.feature = random.choice(
            available_features) if available_features else None
        self.threshold = torch.rand(
            1).item() if self.feature is not None else None
        ...
        if not self.is_leaf:
            split_point = self.feature_ranges[self.feature][0] + self.threshold * (
                self.feature_ranges[self.feature][1] - self.feature_ranges[self.feature][0])
            ...
            remaining_features = [
                f for f in available_features if f != self.feature]
            self.left = RandomDecisionTreeNode(
                depth + 1, max_depth, n_features, left_ranges, remaining_features)
            self.right = RandomDecisionTreeNode(
                depth + 1, max_depth, n_features, right_ranges, remaining_features)
        else:
            self.left = None
            self.right = None
    ...
    def generate_vector(self, return_leaf=True):
        if self.is_leaf:
            vector = torch.rand(self.n_features).mul(
                self.feature_ranges[:, 1] - self.feature_ranges[:, 0]) + self.feature_ranges[:, 0]
    ...
    def compute_leaf_averages(self, averages=None, leaves=None):
        ...
        if self.is_leaf:
            if self.data:
                ...
                # Average across all dimensions for each vector
                leaf_average = leaf_data.mean(dim=0)
...
class DTDataset(data.Dataset):
    ...
    def get_DT_matrix(self, m, n, depth):
        ...
        root = RandomDecisionTreeNode(
            0, depth, n, initial_ranges, available_features)
        vectors = []
        leaf_nodes = []
        for _ in range(m):
        ...
        leaf_to_average = {leaf: avg for leaf,
                           avg in zip(leaves, leaf_averages)}

        averages_for_each_vector = torch.tensor(
            [leaf_to_average[leaf].mean().item() for leaf in leaf_nodes])

        # Concatenate this tensor of averages with the original tensor_vectors
        tensor_vectors_with_averages = torch.cat(
            (tensor_vectors, averages_for_each_vector.unsqueeze(1)), dim=1)
